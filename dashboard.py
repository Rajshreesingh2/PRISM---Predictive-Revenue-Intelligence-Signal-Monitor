...
# ── Absolute base path — works on Streamlit Cloud AND locally ──
...
section[data-testid="stSidebar"] .stMetric {
    background: rgba(100,181,246,0.07); border-radius: 10px; padding: 10px;
...
.glass-card {
    ...
    border: 1px solid rgba(178,223,219,0.5); border-radius: 18px;
    padding: 24px 18px; box-shadow: 0 4px 24px rgba(13,27,42,0.07);
    margin-bottom: 14px; text-align: center;
    transition: transform 0.2s, box-shadow 0.2s;
}
.glass-card:hover { transform: translateY(-3px); box-shadow: 0 12px 40px rgba(13,27,42,0.11); }
...
.sec-header {
    ...
    margin:26px 0 13px; padding:10px 16px;
    background:linear-gradient(90deg,rgba(2,119,189,0.08),rgba(0,131,143,0.05));
    border-left:4px solid #0277bd; border-radius:0 10px 10px 0;
}
.insight-glass {
    background:rgba(255,255,255,0.88); backdrop-filter:blur(10px);
    border-radius:14px; padding:18px 20px;
    box-shadow:0 3px 16px rgba(13,27,42,0.06); margin-bottom:12px;
}
...
.ai-chip {
    display:inline-block; background:linear-gradient(135deg,#e1f5fe,#e0f2f1);
    color:#0277bd; padding:3px 11px; border-radius:20px;
    font-size:0.7rem; font-weight:700; letter-spacing:0.05em; margin-bottom:10px;
...
# ── load data — ALL paths use BASE ───────────────────────────
@st.cache_data
def load():
    d = {}
    try:
        d["telco"] = pd.read_csv(os.path.join(DATA, "telco_cleaned.csv"))
    except Exception as e:
        st.error(f"Could not load telco_cleaned.csv: {e}")
        d["telco"] = None
    try:
        p = pd.read_csv(os.path.join(DATA, "predictions_with_roi.csv"))
        d["preds"] = p.reset_index(drop=True)
    except:
        d["preds"] = None
    try:
        d["arch"] = json.load(open(os.path.join(DATA, "archetype_summary.json")))
    except:
        d["arch"] = {}
    try:
        d["macro"] = json.load(open(os.path.join(DATA, "macro_signals.json")))
    except:
        d["macro"] = {}
    try:
        ab = json.load(open(os.path.join(DATA, "ab_test_results.json")))
        d["ab"] = ab.get("results", ab)
    except:
        d["ab"] = {}
    try:
        d["models"] = json.load(open(os.path.join(MODELS, "model_results.json")))
    except:
        d["models"] = {}
    try:
        d["mi"] = pd.read_csv(os.path.join(DATA, "feature_importance_mi.csv"))
    except:
        d["mi"] = None
    return d
...
with st.sidebar:
    st.markdown("""<div style='text-align:center;padding:20px 0 12px'>
        ...
        <div style='font-size:1.5rem;font-weight:800;background:linear-gradient(135deg,#e3f2fd,#b2ebf2);
        -webkit-background-clip:text;-webkit-text-fill-color:transparent;letter-spacing:0.08em'>PRISM</div>
    ...
    if telco is not None:
        n = len(telco); ch = int(telco["Churn_binary"].sum())
    ...
    st.markdown("""<div style='font-size:0.75rem;color:#9fa8da;line-height:2'>
        ...
        <a href='https://github.com/Rajshreesingh2/PRISM---Predictive-Revenue-Intelligence-Signal-Monitor'
        style='color:#90caf9'>GitHub →</a>
    </div>""", unsafe_allow_html=True)
    ...
    if telco is not None:
        ...
        with c1:
            ...
            fig = go.Figure(go.Bar(
                ...
                marker=dict(color=["#3f51b5","#f9a825","#00897b"], opacity=0.9, line=dict(width=0)),
                text=grp["Rate"].apply(lambda x: f"<b>{x:.1f}%</b>"), textposition="outside"
            ))
            ...
            insight("blue","What this means",
                "Month-to-month customers churn at 42.7% — nearly 7x the rate of two-year contract customers. Contract type is the single strongest predictor in the entire dataset.")

        with c2:
            igrp = telco[telco["Churn_binary"]==1].groupby("InternetService")["MonthlyCharges"].sum().reset_index()
            fig2 = go.Figure(go.Pie(
                labels=igrp["InternetService"], values=igrp["MonthlyCharges"], hole=0.6,
            ...
            insight("red","What this means",
                "Fiber optic customers represent the largest share of lost MRR. They pay more per month AND churn more — the premium segment is your highest financial risk.")
        ...
        with c3:
            ...
            fig3 = go.Figure(go.Bar(
                ...
                marker=dict(color=tgrp["Rate"],
                    colorscale=[[0,"#c8e6c9"],[0.4,"#fff9c4"],[1,"#ffcdd2"]], line=dict(width=0)),
                text=tgrp["Rate"].apply(lambda x: f"<b>{x:.1f}%</b>"), textposition="outside"
            ))
            fig3.update_layout(**pl("Churn by Tenure", 320, yaxis=dict(range=[0,65],gridcolor="#e8eaf6")))
            st.plotly_chart(fig3, use_container_width=True)
            insight("amber","What this means",
                "New customers (0-6 months) churn at 52.9% — more than half leave before month 7. After 2 years, churn drops below 10%.")
        ...
        with i1:
            st.markdown("""<div class="insight-glass purple">
                ...
                <div class="insight-body-c">Customers on month-to-month contracts who also use fiber optic internet churn at <b>52.4%</b> — the highest risk combination. This segment alone represents <b>$47,000+/year</b> in preventable MRR loss.</div>
            </div>""", unsafe_allow_html=True)
        with i2:
        ...
        with i3:
            st.markdown("""<div class="insight-glass amber">
                ...
                <div class="insight-body-c">Customers using <b>4+ services</b> churn at just <b>8.3%</b> vs <b>32%</b> for single-service users. Every additional service adopted reduces churn probability by ~4pp.</div>
            </div>""", unsafe_allow_html=True)
    else:
        st.error("❌ telco_cleaned.csv not found. Check that data/ folder is committed to GitHub.")
        st.code(f"Looking in: {DATA}")
...
# ════════════════════════════════════════════════════════════
    ...
    if preds is not None and telco is not None:
        ...
        if "monthly_charges" not in df2.columns:
        ...
        with c1:
            ...
            st.plotly_chart(fig, use_container_width=True)
            insight("blue","What this means",
                "Most customers cluster at low risk (<30%) with a long tail of high-risk customers. The goal is to intervene with the high-probability tail before they actually churn.")

        with c2:
            fig2 = go.Figure(go.Scatter(
                x=df2["churn_probability"], y=df2["monthly_charges"], mode="markers",
                marker=dict(color=df2["churn_probability"], colorscale="RdYlGn_r",
                    size=6, opacity=0.6, colorbar=dict(title="Churn Prob"))
            ))
            fig2.update_layout(**pl("Churn Probability vs Monthly Charges", 320,
                xaxis_title="Churn Probability", yaxis_title="Monthly Charges ($)"))
            st.plotly_chart(fig2, use_container_width=True)
            insight("amber","What this means",
                "High-charge customers with high churn probability are your most valuable intervention targets — they have the highest CLV at risk.")
    else:
        st.warning("predictions_with_roi.csv not found in data/ folder.")
...
# ════════════════════════════════════════════════════════════
    ...
    if arch:
        ...
        c1,c2 = st.columns(2)
        with c1:
            adf = pd.DataFrame([{"Archetype":k,"MRR":v["mrr_at_risk"],"N":v["count"]} for k,v in arch.items()])
            fig = go.Figure(go.Bar(
                ...
                text=adf["MRR"].apply(lambda x: f"<b>${x:,.0f}</b>"), textposition="outside"
            ))
            fig.update_layout(**pl("Monthly Revenue at Risk by Archetype", 380))
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig2 = go.Figure(go.Pie(
        ...
        sec("🎯 Intervention Playbook")
        strategies = {
            "Price Refugee":     ("#e53935","💰 Loyalty Discount","Offer 15-20% bill reduction or a plan downgrade. ROI is 5-8x. Act within 30 days of risk signal."),
            "Early Dropout":     ("#f9a825","📞 Proactive Onboarding","Trigger CS outreach on days 7, 30, and 60. Feature walkthroughs reduce early churn by 15-20%."),
            "Tech Dissatisfied": ("#7b1fa2","🔧 Free Tech Support Upgrade","Offer a free 3-month TechSupport add-on. Cost: ~$5/month per customer."),
            "Lifecycle Leaver":  ("#00897b","📋 Proactive Contract Renewal","Send a renewal incentive 60 days before contract expiry. Retains ~40% of this cohort."),
        }
        sc = st.columns(2)
        for i,(arch_name,(color,action,desc)) in enumerate(strategies.items()):
            with sc[i%2]:
                st.markdown(f"""<div class="insight-glass" style="border-left:5px solid {color};margin-bottom:14px">
                    ...
                    <div style="font-size:0.72rem;font-weight:700;color:#90a4ae;text-transform:uppercase;
                    letter-spacing:0.07em;margin:4px 0 10px">{arch_name}</div>
                    <div class="insight-body-c">{desc}</div>
                </div>""", unsafe_allow_html=True)
    else:
        st.warning("archetype_summary.json not found in data/ folder.")
...
# ════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="page-sub">Interactive 3D scatter — explore customer segments across multiple dimensions simultaneously</div>', unsafe_allow_html=True)
    ...
    if telco is not None:
        ...
        fig = go.Figure(go.Scatter3d(
            x=sample["tenure"], y=sample["MonthlyCharges"], z=sample["base_risk_score"],
            mode="markers",
            marker=dict(size=4, color=sample["Churn_binary"],
                colorscale=[[0,"#3f51b5"],[1,"#e53935"]], opacity=0.75,
        ...
        fig.update_layout(
            ...
            paper_bgcolor="rgba(255,255,255,0.85)", height=580, margin=dict(l=0,r=0,t=50,b=0)
        )
        st.plotly_chart(fig, use_container_width=True)
        ...
        fig3 = go.Figure(go.Heatmap(
            ...
            textfont=dict(color="#1a237e",size=12), colorbar=dict(title="Churn Rate %")
        ))
        fig3.update_layout(**pl("Churn Rate Heatmap: Contract × Internet Service", 320))
        st.plotly_chart(fig3, use_container_width=True)
        insight("red","The hotspot revealed",
            "Month-to-month Fiber Optic customers churn at 52%+ — the highest risk combination. This is where $1 of intervention generates the highest return.")
    else:
        st.warning("telco_cleaned.csv not found.")
...
# ════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="page-sub">Cox Proportional Hazards model — the gold standard for time-to-event prediction</div>', unsafe_allow_html=True)
        ...
        try:
            ...
            with c2:
                for color,title,body in [
                    ("red","Month-to-month drops fast","50% of customers churn by month 15. Needs immediate intervention."),
                    ("blue","One-year shows a cliff at 12m","Survival holds then drops sharply at renewal. Intervene 60 days before month 12."),
                    ("green","Two-year is anchor segment","Stays above 85% through month 48. Focus resources elsewhere."),
                ]:
            ...
            for ax_i, col_name in enumerate(["Contract","InternetService"]):
                ...
                fig3 = go.Figure(go.Bar(
                    ...
                    marker=dict(color=sdf["Months"],
                        colorscale=[[0,"#ffcdd2"],[0.5,"#fff9c4"],[1,"#c8e6c9"]], line=dict(width=0)),
                ...
                fig3.update_layout(**pl(f"Median Survival by {col_name}", 320,
                    yaxis=dict(range=[0,85], title="Months until 50% churned")))
                with sc2[ax_i]:
                    st.plotly_chart(fig3, use_container_width=True)

            insight("purple","Why this matters",
                "Month-to-month customers surviving ~8 months at $65/month = $520 CLV. Two-year contract customers surviving 48+ months = $3,120+ CLV. This 6x difference justifies spending significantly more to acquire long-term contract customers.")
        except ImportError:
            st.error("lifelines not installed. Add it to requirements.txt")
    else:
        st.warning("telco_cleaned.csv not found.")
...
# ════════════════════════════════════════════════════════════
    ...
    if ab:
        ...
        with c1:
            fig = go.Figure(go.Bar(
                x=["Control Group","Treatment Group"], y=[cr*100, tr*100],
            ...
            fig.update_layout(**pl("Churn Rate: Control vs Treatment", 340,
                yaxis=dict(range=[0,65], title="Churn Rate (%)")))
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            ...
            <tr style="background:linear-gradient(90deg,#e8eaf6,#ede7f6)">
                <th style="padding:10px;color:#1a237e">Test</th>
            ...
            <tr style="border-bottom:1px solid #e8eaf6">
                <td style="padding:12px;color:#546e7a">Chi-square test</td>
                <td style="padding:12px;text-align:center;font-weight:700;color:#1a237e">{p1:.4f}</td>
                <td style="padding:12px;text-align:center">
                <span style="background:{sig1[1]};color:{sig1[2]};padding:4px 12px;border-radius:20px;
                font-size:0.78rem;font-weight:700">{sig1[0]}</span></td>
            </tr>
            <tr>
                <td style="padding:12px;color:#546e7a">Z-test proportions</td>
                <td style="padding:12px;text-align:center;font-weight:700;color:#1a237e">{p2:.4f}</td>
                <td style="padding:12px;text-align:center">
                <span style="background:{sig2[1]};color:{sig2[2]};padding:4px 12px;border-radius:20px;
                font-size:0.78rem;font-weight:700">{sig2[0]}</span></td>
            </tr>
            </table></div>""", unsafe_allow_html=True)

        insight("purple","What is Difference-in-Differences?",
            "DiD compares the CHANGE in churn rate for treatment vs control. It controls for pre-existing differences — this is the causal inference method used by Google, Airbnb, and Visa.")
        insight("green","Why this is production-grade thinking",
            "Most DS projects show 'model accuracy = 87%'. PRISM goes further: we designed an experiment, randomized a control group, ran statistical tests, and estimated the causal effect. This is the full DS workflow.")
    else:
        st.warning("ab_test_results.json not found in data/ folder.")
    ...
    if mr:
        ...
        if mi_df is not None:
            ...
            fig2 = go.Figure(go.Bar(
                x=top_mi["mi_score"], y=top_mi["feature"], orientation="h",
                marker=dict(color=top_mi["mi_score"],
                    colorscale=[[0,"#c5cae9"],[0.5,"#7986cb"],[1,"#3f51b5"]], line=dict(width=0)),
                text=top_mi["mi_score"].apply(lambda x: f"{x:.4f}"), textposition="outside"
            ))
            fig2.update_layout(**pl("Top 15 Features by Mutual Information Score", 450,
                xaxis_title="MI Score"))
            st.plotly_chart(fig2, use_container_width=True)

        sec("⚖️ Temporal vs Random Split")
        c1,c2 = st.columns(2)
        with c1:
            insight("red","❌ Random Split (what most portfolios do)",
                "train_test_split(random_state=42) leaks future data. AUC inflates to 0.92+. Fails immediately in production.")
        with c2:
            insight("green","✅ Temporal Split (what PRISM does)",
                "Sort by tenure → train on short-tenure, test on long-tenure. Honest 0.79 AUC. Reflects true production performance.")
    else:
        st.warning("models/model_results.json not found.")
...
# ════════════════════════════════════════════════════════════
    ...
    if macro:
        ...
        sec("🌡️ Consumer Stress Index")
        c1,c2 = st.columns([1,2])
        with c1:
            fig = go.Figure(go.Indicator(
                mode="gauge+number+delta", value=round(stress,2),
                delta={"reference":5,"increasing":{"color":"#e53935"},"decreasing":{"color":"#00897b"}},
                title={"text":"<b>Stress Index</b><br><span style='font-size:11px'>Inflation+Unemployment−GDP</span>",
        ...
        with c2:
            insight("blue","Consumer Stress Index",
                "Stress = Inflation + Unemployment − GDP. Above 8: telecom churn increases 8-12% industry-wide. Current reading is relatively benign.")
            insight("amber","Google Trends Signals",
                f"'Cancel phone plan' intent score: <b>{ci:.1f}/100</b>. 'Switch carrier' intent: <b>{sw:.1f}/100</b>. Leading indicators that appear weeks before actual churn.")
            insight("green","Why this differentiates PRISM",
                "No other portfolio project combines macroeconomic signals with individual customer predictions. This is what a senior DS at Visa or Mastercard would build.")

        sec("📋 All Macro Signals")
        mdf2 = pd.DataFrame([
        ...
        st.warning("macro_signals.json not found in data/ folder.")

# ════════════════════════════════════════════════════════════
# PAGE 9 — Behavioral Analytics
# ════════════════════════════════════════════════════════════
elif "Behavioral" in page:
    st.markdown('<div class="page-title">📱 Behavioral Analytics</div>', unsafe_allow_html=True)
    st.markdown('<div class="page-sub">Feature usage patterns, engagement signals, and service adoption analysis</div>', unsafe_allow_html=True)

    if telco is not None:
        sec("🔧 Service Adoption: Churned vs Retained")
        service_cols = [c for c in ["OnlineSecurity","OnlineBackup","DeviceProtection","TechSupport","StreamingTV","StreamingMovies"] if c in telco.columns]
        if service_cols:
            churn_rates = []
            for svc in service_cols:
                col_data = telco[svc]
                adopted = (col_data == "Yes") if col_data.dtype == object else col_data.astype(bool)
                churn_rates.append({"Service": svc.replace("_"," "),
                    "With Service": telco[adopted]["Churn_binary"].mean()*100,
                    "Without Service": telco[~adopted]["Churn_binary"].mean()*100})
            ...
            fig.update_layout(**pl("Churn Rate: Adopted vs Not Adopted per Service", 400, barmode="group",
                yaxis=dict(range=[0,50], title="Churn Rate (%)")))
            st.plotly_chart(fig, use_container_width=True)
            insight("green","What this reveals",
                "Every service adopted reduces churn probability. OnlineSecurity and TechSupport show the strongest protective effect. Cross-selling is not just a revenue play — it's your strongest retention lever.")

        sec("👥 Customer Profile: Churned vs Retained")
        c1,c2 = st.columns(2)
        with c1:
            profile_features = [f for f in ["SeniorCitizen","Partner","Dependents","PhoneService","PaperlessBilling"] if f in telco.columns]
            ...
            fig2.update_layout(**pl("Demographics: Churned vs Retained (%)", 360, barmode="group",
                yaxis=dict(range=[0,100], title="% of segment")))
            st.plotly_chart(fig2, use_container_width=True)
            insight("amber","Key demographic pattern",
                "Churned customers are less likely to have partners or dependents — customers with families have higher switching costs.")
        ...
        with c2:
            ...
            fig3 = go.Figure(go.Bar(
                x=pay_churn["Rate"], y=pay_churn["Method"], orientation="h",
                marker=dict(color=pay_churn["Rate"],
                    colorscale=[[0,"#c8e6c9"],[0.5,"#fff9c4"],[1,"#ffcdd2"]], line=dict(width=0)),
                text=pay_churn["Rate"].apply(lambda x: f"<b>{x:.1f}%</b>"), textposition="outside"
            ))
            fig3.update_layout(**pl("Churn Rate by Payment Method", 360,
                xaxis=dict(range=[0,50], title="Churn Rate (%)")))
            st.plotly_chart(fig3, use_container_width=True)
            insight("red","Auto-pay is a retention moat",
                "Electronic check customers churn at 45%. Auto-pay customers churn at just 15-17%. Nudging customers to switch to auto-pay is one of the cheapest retention interventions.")
    else:
        st.warning("telco_cleaned.csv not found.")

# ════════════════════════════════════════════════════════════
# PAGE 10 — Cohort Retention
# ════════════════════════════════════════════════════════════
elif "Cohort" in page:
    st.markdown('<div class="page-title">🔄 Cohort Retention Analysis</div>', unsafe_allow_html=True)
    st.markdown('<div class="page-sub">How different customer groups retain over time — foundation of subscription business health measurement</div>', unsafe_allow_html=True)
    ...
    if telco is not None:
        ...
        cohort_data = telco.groupby("cohort", observed=True).agg(
            total=("Churn_binary","count"), churned=("Churn_binary","sum")
        ).reset_index()
        ...
        fig = go.Figure(go.Heatmap(
            ...
            texttemplate="%{text}", textfont=dict(color="#1a237e",size=13),
            colorbar=dict(title="Retention %"), zmid=75
        ))
        ...
        with c1:
            ...
            fig2.update_layout(**pl("Cohort Size: Retained vs Churned", 360,
                barmode="stack", yaxis=dict(title="Customers")))
            st.plotly_chart(fig2, use_container_width=True)
        with c2:
            fig3 = go.Figure(go.Scatter(
                x=cohort_data["cohort"].astype(str), y=cohort_data["retained_pct"],
                mode="lines+markers+text", line=dict(color="#3f51b5",width=3),
            ...
            fig3.update_layout(**pl("Retention Rate Curve by Cohort", 360,
                yaxis=dict(range=[0,110], title="Retention Rate (%)")))
            st.plotly_chart(fig3, use_container_width=True)

        insight("amber","The retention story",
            "Longer-tenure cohorts retain dramatically better. The 0-6 month window is the danger zone — this is where most preventable churn occurs and where retention investment has the highest ROI.")
...
# ════════════════════════════════════════════════════════════
# PAGE 11 — Revenue Analytics
# ════════════════════════════════════════════════════════════
elif "Revenue" in page:
    st.markdown('<div class="page-title">💰 Revenue Analytics</div>', unsafe_allow_html=True)
    st.markdown('<div class="page-sub">MRR distribution, customer lifetime value, and revenue concentration analysis</div>', unsafe_allow_html=True)
    ...
    if telco is not None:
        ...
        with c1:
            ...
            fig.update_layout(**pl("Monthly Charges Distribution", 360, barmode="overlay",
                xaxis_title="Monthly Charges ($)", yaxis=dict(title="Customers")))
            st.plotly_chart(fig, use_container_width=True)
            insight("red","The high-value paradox",
                "Churned customers tend to have HIGHER monthly charges. Revenue loss from churn is disproportionate to the number of customers lost.")
        with c2:
            telco["charge_bucket"] = pd.cut(telco["MonthlyCharges"],
                bins=[0,30,50,70,90,120], labels=["<$30","$30-50","$50-70","$70-90","$90+"])
            charge_analysis = telco.groupby("charge_bucket",observed=True).agg(
                count=("Churn_binary","count"), churn_rate=("Churn_binary","mean")
            ).reset_index()
            charge_analysis["churn_rate"] *= 100
            fig4 = go.Figure(go.Bar(
                x=charge_analysis["charge_bucket"].astype(str), y=charge_analysis["churn_rate"],
        ...
        insight("purple","Revenue concentration risk",
            f"Total annual revenue at risk: ${churned_mrr*12:,.0f}. Losing a high-charge customer is 3-4x more damaging — this is why CLV-weighted intervention priority matters.")
    else:
        st.warning("telco_cleaned.csv not found.")

# ════════════════════════════════════════════════════════════
# PAGE 12 — Segmentation Explorer
# ════════════════════════════════════════════════════════════
    ...
    if telco is not None:
        svc_cols = [c for c in ["OnlineSecurity","OnlineBackup","DeviceProtection","TechSupport","StreamingTV","StreamingMovies"] if c in telco.columns]
        if svc_cols:
            def count_services(row):
                return sum(1 for c in svc_cols if str(row[c]) == "Yes" or row[c] == 1)
            telco["rfm_frequency"] = telco.apply(count_services, axis=1)
        else:
            telco["rfm_frequency"] = 0

        telco["r_score"] = pd.qcut(telco["tenure"],         q=3, labels=[1,2,3]).astype(int)
        telco["f_score"] = pd.cut(telco["rfm_frequency"],   bins=[-1,1,3,6], labels=[1,2,3]).astype(int)
        telco["m_score"] = pd.qcut(telco["MonthlyCharges"], q=3, labels=[1,2,3]).astype(int)
        ...
        seg_summary = telco.groupby("rfm_segment").agg(
            count=("Churn_binary","count"), churn_rate=("Churn_binary","mean"),
            avg_mrr=("MonthlyCharges","mean"), total_mrr=("MonthlyCharges","sum")
        ).reset_index()
        ...
        with c1:
            ...
            fig.update_layout(**pl("Churn Rate by RFM Segment", 360,
                yaxis=dict(range=[0,55], title="Churn Rate (%)")))
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig2 = go.Figure(go.Pie(
                labels=seg_summary["rfm_segment"], values=seg_summary["total_mrr"], hole=0.55,
        ...
        selected_seg = st.selectbox("Select a segment to explore", sorted(telco["rfm_segment"].unique()))
        ...
        with c2:
            ...
            fig4 = go.Figure(go.Pie(
                labels=internet_dist["Service"], values=internet_dist["Count"], hole=0.5,
        ...
        insight(
            ...
            f"Average tenure: {seg_data['tenure'].mean():.1f} months. Average MRR: ${seg_data['MonthlyCharges'].mean():.0f}. "
            f"{'HIGH PRIORITY — churn rate exceeds 30%.' if seg_data['Churn_binary'].mean()>0.3 else 'Lower risk — focus resources elsewhere.'}"
        )
    else:
        st.warning("telco_cleaned.csv not found.")